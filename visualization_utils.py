...
def visualize_boxes_and_labels_on_image_array(
  ...
  # print("printing array {}".format(i))
  # print(arr)
  # print("End printing array")
  # df=pd.DataFrame([],columns=['class','ymax','ymin','xmax','xmin'])
  class_data=pd.Series([])
  cod_data=pd.DataFrame([],columns=["ymax","ymin","xmax","xmin"])
        ...
        if not skip_labels:
          if not agnostic_mode:
            if classes[i] in category_index.keys():
              ...
              # print(class_name)
              # df1=pd.DataFrame([[class_name,0,0,0,0]],columns=['class','ymax','ymin','xmax','xmin'])
              # df=df.append(df1)
              class_data=class_data.append(pd.Series([class_name]))
            else:
              class_name = 'N/A'
  ...
  for box, color in box_to_color_map.items():
    ...
    draw_bounding_box_on_image_array(
        ...
        use_normalized_coordinates=use_normalized_coordinates)
    # print(ymin,ymax,xmax,xmin)
    df1=pd.DataFrame([[ymax,ymin,xmax,xmin]],columns=['ymax','ymin','xmax','xmin'])
    cod_data=cod_data.append(df1,ignore_index=True)
    # df.iloc[ind]['ymin']=ymin
    # df.iloc[ind]['xmax']=xmax
    # df.iloc[ind]['xmin']=xmin
    # df.iloc[ind]['ymax']=ymax
    # ind=ind+1
    if keypoints is not None:
  ...
  return cod_data, class_data,image
